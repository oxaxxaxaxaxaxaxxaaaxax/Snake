...
func (u *UnicastSocket) ListenMessage(evChan chan domain.Event) {
	buf := make([]byte, 1024)
...
func SendMessage(conn net.PacketConn, addr net.Addr, n int) {
	buf := make([]byte, 1024)
...
func (u *UnicastSocket) SendAck(receiveMsg *pb.GameMessage, address string, senderId, receiverId int32) error {
	var msg []byte
	var err error
...
func (u *UnicastSocket) SendPing(address string, senderId int32) error {
	msg, err := proto.Marshal(&pb.GameMessage{
		MsgSeq:   &u.Seq,
		SenderId: &senderId,
...
func (u *UnicastSocket) SendSteer(address string, senderId int32, direction pb.Direction) error {
	msg, err := proto.Marshal(&pb.GameMessage{
		MsgSeq:   &u.Seq,
		SenderId: &senderId,
...
func (u *UnicastSocket) SendState(address string, senderId int32, state *pb.GameState) error {
	msg, err := proto.Marshal(&pb.GameMessage{
		MsgSeq:   &u.Seq,
		SenderId: &senderId,
...
func (u *UnicastSocket) SendAnnouncement(address string, senderId int32, ann []*pb.GameAnnouncement) error {
...
func (u *UnicastSocket) SendJoin(address string, ctx domain.GameCtx) error {
	msg, err := proto.Marshal(&pb.GameMessage{
		MsgSeq:   &u.Seq,
...
func (u *UnicastSocket) SendError(address string, senderId int32, errorMsg string) error {
	msg, err := proto.Marshal(&pb.GameMessage{
		MsgSeq:   &u.Seq,
		SenderId: &senderId,
...
func (u *UnicastSocket) SendRoleChange(address string, senderId int32, recvRole, sendRole pb.NodeRole) error {
	msg, err := proto.Marshal(&pb.GameMessage{
		MsgSeq:   &u.Seq,
		SenderId: &senderId,
